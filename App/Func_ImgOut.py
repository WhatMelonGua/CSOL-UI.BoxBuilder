...
def boxData2UI(boxData,name='',pos=(0,0),boxMult=1,funcSet=(False,False,False,False),speed=100):       # pos必须事先整型化，函数没内置！cj储存pos和box的信息，是int还是str,如（T，T，F），代表posX、posY是int，boxfactor是str
    ...
    info = "--导出UI.Box表（一个表存储了一个图片的set信息）共"+str(len(colorList))+"个\n--分别占用Box数目为："+str(numList)+"\n--其中单图最大使用数为："+ str(maxNum) +"  | 总消耗Box数目"+str(sum(numList))+"个"
    # 项目说明
    t_funcUse = []
    if funcSet[1] or funcSet[2] or funcSet[3] or funcSet[4]:
        t_funcUse.append("_boxUseNum = 0\n")
    if funcSet[1] == True or funcSet[3] == True:  # 便捷绘制函数添加
        t_funcUse.append("function _boxDraw(boxTable,setTable,pos,mult,add)\n"
                         ...
                         "end\n\n")
    if funcSet[2] or funcSet[4]:
        t_funcUse.append("function _colorAdd(color1,color2)\n"
                         "\tr = (color1[1]+color2[1])%256;g = (color1[2]+color2[2])%256;b = (color1[3]+color2[3])%256;a = color1[4]+color2[4]\n"
                         "\tif r < 0 then r = 256-r end if g < 0 then g = 256-g end if b < 0 then b = 256-b end if a > 255 then a = 255 end\n"
                         "\tres = {r,g,b,a}\n"
                         "\treturn res\nend\n")
    if funcSet[2] == True:  # 色彩进阶绘制函数添加
        t_funcUse.append("function _boxDraw_color(boxTable,setTable,pos,mult,add,color)\n"
                         "\tif add == nil or add == false then _boxUseNum = 0 end\n"
                         "\tif color == nil then color = {0,0,0,0} end\n"
                         "\tfor index,set in pairs(setTable) do\n"
                         "\t\tboxTable[index+_boxUseNum]:Show()\n"
                         "\t\trgba = _colorAdd({set[5],set[6],set[7],set[8]},{color[1],color[2],color[3],color[4]})\n"
                         "\t\tboxTable[index+_boxUseNum]:Set({x=pos[1]+set[1]*mult,y=pos[2]+set[2]*mult,width=set[3]*mult,height=set[4]*mult,r=rgba[1],g=rgba[2],b=rgba[3],a=rgba[4]})\n"
                         "\tend\n"
                         "\t_boxUseNum = _boxUseNum + #setTable\n"
                         "end\n\n")
    if funcSet[4] == True:
        t_funcUse.append("_t_animAct="+str(speed/1000)+"\n"
                         "_anim_frameVal=1\n"
                         "_t_tick=0\n"
                         "function _AnimationImgList(boxTable,setList,pos,mult,color)\n"
                         "\tif color == nil then color = {0,0,0,0} end\n"
                         "\t--确定前后帧的方块用量\n"
                         ...
                         "\t\tboxTable[index]:Show()\n"
                         "\t\trgba = _colorAdd({set[5],set[6],set[7],set[8]},{color[1],color[2],color[3],color[4]})\n"
                         "\t\tboxTable[index]:Set({x=pos[1]+set[1]*mult,y=pos[2]+set[2]*mult,width=set[3]*mult,height=set[4]*mult,r=rgba[1],g=rgba[2],b=rgba[3],a=rgba[4]})\n"
        ...
        for box in range(len(boxList[frame])):
            for set in range(len(boxList[frame][box])):
                ...
                t_set += "{" + str(boxList[frame][box][set][0]) + "," + str(boxList[frame][box][set][1]) + "," + str(boxList[frame][box][set][2]) + "," + str(boxList[frame][box][set][3]) + "," \
                         + str(boxList[frame][box][set][4])+ "," + str(boxList[frame][box][set][5]) + "," \
                         + str(boxList[frame][box][set][6]) + "," + str(boxList[frame][box][set][7]) + "}"
                singleSet += t_set
        singleSet += "}\n"
    ...
    if funcSet[0] == True:  # 色彩偏移计算支持
        t_act.append("function _colorMove(t,base,mult)\n"
                     "\treturn (base+mult*t)%256\n"
                     "end\n")
    if funcSet[3] == True:  # 首图绘制激活
        t_act.append("_boxDraw(" + name + "_bT," + name + "_sT[1],{" + str(pos[0]) + "," + str(pos[1]) + "}," + str(boxMult) + ",false)\n")
    if funcSet[4] == True:  # 动画激活
        t_act.append("function UI.Event:OnUpdate(t)\n"
                     ...
                     "\tend\n"
                     "end\n")
    return (info,t_funcUse,t_boxTable,t_setTable,t_act)
# python数组转txt的Lua文本
