    ...
    def create():
        ...
        if outputPath == "":
            ...
            if Out.exp(num_Entry.get()) == 'num':
                ...
                return
            # 检查数字
            cont = str(name_tk.get()) + "_boxTable = {"
...
disposeMenu.add_command(label='动图导出')
exportMenu = tk.Menu(menuBar,tearoff=0)
menuBar.add_cascade(label='导出设置',menu=exportMenu)
exportMenu.add_checkbutton(label='支持色彩偏移计算方法', variable=fastAddCode)
exportMenu.add_checkbutton(label='添加单图绘制函数', variable=drawCode)
exportMenu.add_checkbutton(label='添加追加绘制函数', variable=addDrawCode)
